...
/**
 * Deterministic, in-memory {@link IFurnaceManagerAccessor} for T2 tests — no MC statics, so tests can
 ...
 * (TE's {@code ComparableItemStackFurnace}/{@code RecipeFurnace} are not on the JUnit test classpath), so
 * this fake holds a raw {@link Map}; {@code null} is allowed to exercise the null-safe path.
 */
public final class FakeFurnaceManagerAccessor implements IFurnaceManagerAccessor {

    private final Map recipeMap;

    @SuppressWarnings("rawtypes")
    public FakeFurnaceManagerAccessor(final Map recipeMap) {
